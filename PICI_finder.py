...
def cpu_split(seqfile: click.Path, cpu: int, length: int, out: click.Path):
    """
    PICIfinder Main Function.

    Parameters:
        seqfile: Input FASTA file containing genomic sequences.
        cpu: Number of parallel processes to use.
        length: Minimum sequence length threshold for analysis.
        out: Output directory path for results.
    ...
    try:
        # Extract filename and path components from the input sequence file path
        filename_resuf = str(seqfile.rsplit('/')[-1].rsplit(".",1)[0])
    except Exception:
        # Fallback: use the full path as filename if path parsing fails
        filename_resuf = str(seqfile)

    # Convert sequences exceeding length limit from FASTA to TXT and standardize sequence names
    sequences = 0
    ...
    # Gene annotation and feature extraction
    n = 0
    p_list = []
    while n < len(parallels2):
        judge = 'python3 '+PICI_finder_path+'/scripts/PICI_Feature_Extraction.py ' + parallels2[n] + ' ' + str(cpu) + ' ' + str(n)
    ...
    # Result integration and post-processing
    subprocess.run('python3 '+PICI_finder_path+'/scripts/PICI_Result_Integration.py '+out, shell=True)
