"""
    ...
    PICI_Feature_Extraction.py

    Functional Overview:
        This module serves as the feature extraction pipeline of the PICI_finder tool, responsible for
        extracting gene features, protein domains, and functional annotation information from input FASTA sequence files,
        providing the data foundation for subsequent PICI identification.

    Processing Flow:
        1. Gene Prediction and Annotation
           - Prokka: Gene functional annotation
           - Prodigal: ORF open reading frame prediction
        2. Protein Domain Search
           - HMMER: PICI characteristic domains, T4SS system, bacteria-specific domain searches
           - DIAMOND: Sequence similarity alignment
        3. Data Integration and Cleanup
           - Integration of ORF, HMMER, and Prokka results
           - Generation of unified format feature tables
           - Cleanup of temporary files

    Dependencies:
    - Prokka, Prodigal, HMMER, DIAMOND
"""
...
def orf_hmm_prokka_diamond_find(filename: str, seq_file_name: str, threads: str, PICI_finder_path: str):
    """
    Analysis pipeline for performing gene prediction and protein domain search,
    generating Prokka annotation files, ORF prediction files, HMMER search results,
    and DIAMOND alignment results.

    Parameters:
        filename: Path to the input FASTA file.
        seq_file_name: Base filename (without extension) used for output files.
        threads: Number of parallel threads/processes.
        PICI_finder_path: Path to the PICI finder models and databases.
    ...
    # PICI Characteristic Domain Search
    p_list = []
    ...
    p_list.append(hmm2)
    # Bacteria-specific Domain Search
    ...
    # DIAMOND Sequence Similarity Search
    time = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    if int(threads) == 1:
    ...
    # DIAMOND Search Against PICI Characteristic Database
    ...
    # DIAMOND Search Against Phage Protein Database
...
def orf_file_deal(seq_file_name: str):
    """
    Process ORF prediction results file and organize into structured data.

    Parameters:
    seq_file_name: Path to the directory/file prefix for input and output files

    return: DataFrame - ORF feature information.
    """

    orf_name_list = []
    diamond_list = []  # Store DIAMOND Alignment Results
    name_list = []  # Store Gene Annotation Names
    gene_ano = ''  # Temporarily Store Gene Annotation

    # Parse ORF File and Convert to DataFrame
    ...
    orf_name_data.columns = ['name','start', 'end', 'dir','mix', 'seq_number', 'gene_number','all_name'] # 列名



    # DIAMOND Alignment Results Processing
    if os.path.getsize(seq_file_name + '_diamond_blast.txt') != 0:
       with open(seq_file_name + '_diamond_blast.txt') as diamond_outcome:
           for line in diamond_outcome.readlines():
               ...
               if ((line.split()[1] != 'phage_resistance' or (line.split()[1] == 'phage_resistance'
                                                              and int(line.split()[3]) < 300))
                       and line.split()[1] != 'stl_gene' and line.split()[1] != 'PICI_gene'
                       and line.split()[1] != 'tail_gene' and line.split()[1] != 'head_gene'
                       and line.split()[1] != 'lysis_gene' and line.split()[1] != 'capsid_gene'
                       and float(line.split()[2]) < 90):
                   gene_ano = 'na'
...
def hmm_file_deal(seq_file_name: str):
    """
    Process HMMER search results and organize protein domain information.

    Parameters:
        seq_file_name: str - Path prefix for input/output files.

    return: pandas.DataFrame - Structured domain prediction data.
    """

    hmm_list = []  # Store HMMER results for PICI and T4SS
    hmm_bac_list = []  # Store HMMER results for bacterial domains

    # Parse HMMER result files
    ...
    # Data cleaning and file content standardization
    ...
    hmm_list.to_csv(seq_file_name + '_hmm_list.tsv', sep='\t', index=False)
    ...
    hmm_file_deal.columns = [
        'structure_name', 'structure_id', 'seq_name', 'E_value', 'start',
        'end', 'dire', 'site','seq_number','domain_start','domain_end'
    ]

    return hmm_file_deal

def prokka_file_deal(seq_file_name: str):
    """
    Process Prokka annotation results and extract gene functional information.

    Parameters:
        seq_file_name: str - Path prefix for input/output files.

    return: DataFrame - Prokka gene annotation information.
    """

    prokka_ext = []

    # Parse GFF file and construct DataFrame
    ...
    # Detailed parsing of gene annotation information in the attribute column
    n = -1
...
if __name__ == "__main__":
    ...
    fasta_name = pd.concat([
        pd.DataFrame(name_list),
        pd.DataFrame(name_count),
        pd.DataFrame(name_length)
    ], axis=1)
    fasta_name.to_csv(seq_file_name + '_fasta_name.txt', sep='\t', index=False)



    # Execute bioinformatics analysis pipeline and process sequence feature results
    orf_hmm_prokka_diamond_find(filename,seq_file_name,threads,PICI_finder_path)
    ...
    orf_table = orf_file_deal(seq_file_name)
    hmm_table = hmm_file_deal(seq_file_name)
    prokka_table = prokka_file_deal(seq_file_name)



    # Generate unique identifiers for merging feature tables
    # Format: sequence_number_start_end_direction
    orf_table['temp'] = '_'
    ...
    # Create master feature table and add identification markers(bacterial and phage)
    ...
    os.remove(seq_file_name + '_hmm_list.tsv')
